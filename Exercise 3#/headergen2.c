...
static void comment(size_t level, const char *com) {
    ...
    for (size_t curr_pos = 0; curr_pos < comm_len; curr_pos += break_used) {
        ...
        if (((nl_pos != NULL) && (nl_pos < space_pos)) || space_pos == NULL)
            space_pos = nl_pos;
        if (space_pos == NULL)
            space_pos = com + max_segment_len;
...
static void snapshot_message2(size_t indent_level, const char *prefix, const struct osc_message *msg) {
    MAKE_INDENT(indent_level);
    printf("%svoid example_msg_%s() {\n", indent, prefix);
    SETUP_INDENT(++indent_level);

    // first - 4 bytes are message length
    const uint8_t *data = msg->raw_data;
    size_t pos_limit = osc_message_serialized_length(msg) + sizeof(uint32_t);

    printf("%suint8_t osc_msg_%s_raw_data[%lu] = {\n", indent, prefix, pos_limit);
    SETUP_INDENT(++indent_level);
    ...
    printf("%sstruct osc_message osc_msg_%s;\n", indent, prefix);
    printf("%sosc_msg_%s.raw_data = osc_msg_%s_raw_data;\n", indent, prefix, prefix);
    printf("%sosc_msg_%s.address = (char *)osc_msg_%s_raw_data + %zu;\n", indent, prefix, prefix, (size_t)((uint8_t *)(msg->address) - data));
    printf("%sosc_msg_%s.typetag = (char *)osc_msg_%s_raw_data + %zu;\n\n", indent, prefix, prefix, (size_t)((uint8_t *)(msg->typetag) - data));
...
static void snapshot_bundle2(size_t indent_level, const char *prefix, const struct osc_bundle *bnd) {
    MAKE_INDENT(indent_level);
    printf("%svoid example_bnd_%s() {\n", indent, prefix);
    SETUP_INDENT(++indent_level);

    // first - 4 bytes are message length
    const uint8_t *data = bnd->raw_data;
    size_t pos_limit = osc_bundle_serialized_length(bnd) + sizeof(uint32_t);

    printf("%suint8_t osc_bnd_%s_raw_data[%lu] = {\n", indent, prefix, pos_limit);
    SETUP_INDENT(++indent_level);
    ...
    printf("%sstruct osc_bundle osc_bnd_%s;\n", indent, prefix);
    printf("%sosc_bnd_%s.raw_data = osc_bnd_%s_raw_data;\n", indent, prefix, prefix);
    printf("%sosc_bnd_%s.timetag = (struct osc_timetag *)(osc_bnd_%s_raw_data + %zu);\n\n", indent, prefix, prefix, (size_t)((uint8_t *)(bnd->timetag) - data));
...
int message_examples() {
    ...
    snapshot_message2(0, "msg_empty", &msg_ref);
    ...
    snapshot_message2(0, buffer, &msg_ref);
...
static void help_run() {
    fprintf(stderr, "Vitejte v hlavnim menu. Zde mate malou napovedu:\n");
    fprintf(stderr, "%c zobrazi napovedu\n", MENU_MAIN_HELP);
    fprintf(stderr, "%c ukonci program\n", MENU_MAIN_QUIT);
    fprintf(stderr, "%c odesle pripravena data\n", MENU_MAIN_GENERATE);
    fprintf(stderr, "%c zalozi novy bundle (rozpracovana data jsou znicena)\n", MENU_MAIN_BUNDLE);
    fprintf(stderr, "%c zalozi novou samostatnou zpravu (rozpracovana data jsou znicena)\n", MENU_MAIN_MESSAGE);
}
static void help_message() {
    fprintf(stderr, "%c zobrazi napovedu\n", MENU_MESSAGE_HELP);
    fprintf(stderr, "%c uloz zpravu a vrat se o uroven vyse\n", MENU_MESSAGE_DONE);
    fprintf(stderr, "%c vypise prave rozpracovanou zpravu\n", MENU_MESSAGE_SHOW);
    fprintf(stderr, "%c nastavi adresu zpravy\n", MENU_MESSAGE_ADDR);
    fprintf(stderr, "%c prida argument typu int\n", MENU_MESSAGE_ADD_INT);
    fprintf(stderr, "%c prida argument typu float\n", MENU_MESSAGE_ADD_FLOAT);
    fprintf(stderr, "%c prida argument typu string\n", MENU_MESSAGE_ADD_STRING);
    fprintf(stderr, "%c prida argument typu osc_timetag\n", MENU_MESSAGE_ADD_TIMETAG);
#ifdef OSC_TT_BLOB
    fprintf(stderr, "%c prida argument typu osc_blob\n", MENU_MESSAGE_ADD_BLOB);
#endif
}
static void help_bundle() {
    fprintf(stderr, "%c zobrazi napovedu\n", MENU_BUNDLE_HELP);
    fprintf(stderr, "%c uloz bundle a vrat se o uroven vyse\n", MENU_BUNDLE_DONE);
    fprintf(stderr, "%c vypise prave rozpracovanou zpravu\n", MENU_BUNDLE_SHOW);
    fprintf(stderr, "%c nastavi timetag\n", MENU_MESSAGE_ADD_TIMETAG);
    fprintf(stderr, "%c prida do bundle zpravu\n", MENU_BUNDLE_ADD_MESSAGE);
}
...
static char *read_string() {
    ...
    fprintf(stderr, "Zadejte radek textu (maximalni delka %u): ", BUFFER_SIZE);
    ...
    if ((content_length > 1) && (buffer[content_length - 1] == '\r')) {
        buffer[content_length - 1] = 0;
    }
    ...
    *msg = calloc(1, sizeof(**msg));
    ...
    do {
        ...
        case MENU_MESSAGE_ADD_BLOB: {
            fprintf(stderr, "Zadany retezec bude interpretovan jako sekvence byte blobu.\n");
            char *str = read_string();
...
static void generate(void *data) {
    if (strncmp((char *) data + sizeof(int32_t), "#bundle", 8) == 0) {
        ...
        snapshot_bundle2(0, hash, &bundle);
    } else {
        ...
        comment(0, comment_buffer);
        snapshot_message2(0, message.typetag + 1, &message);
    }
}
...
int main(int argc, char ** argv) {
    // okay, let's do this
    if (strcmp(argv[0], "examplegen") == 0 || argc > 1)
        message_examples();
    else
        run();
